...
- 右键菜单：支持 `Pin消息/取消Pin`、`复制`、`回复`；单 Agent 聊天中的用户消息和 Agent 回复额外支持 `重新生成`。
- Pin：被 Pin 的消息在聊天流左侧显示 Pin 标识，并进入顶部 `PinnedBar.vue`。
- 复制：复制该消息转换后的纯文本内容。
- 回复：把消息发送者和内容摘要填入输入框上方的回复引用区。
- 重新生成：单 Agent 聊天中基于该消息关联的最近用户输入启动一轮新的后台 turn，不重复追加用户消息；群聊暂不支持。
...
PRD ART-2。`agent-run` 卡片正文下方渲染本回合产出的可预览产物占位卡，点击预览在右侧抽屉查看，点击编辑才展开全屏文件窗口。spec 见 `doc/spec/inline-artifact-preview.md`。
...
- 展示规则：
  ...
  - 点击卡片主体 emit `preview-artifact` → `MessageList` 透传 → `ChatView` 打开 460px `ArtifactPreviewDrawer`（复用 ART-1 的 `getArtifactPreview` + `ArtifactPreviewBody`）。
  - 点击右侧编辑按钮 emit `edit-artifact` → `MessageList` 透传 → `ChatView` 打开全屏 `ArtifactPreviewOverlay`；text/html 以源码 textarea 打开，不渲染 HTML 预览。
- 与黑板侧栏预览（ART-1）保持一致：侧栏入口、内联卡、deploy 卡的预览都打开 460px `ArtifactPreviewDrawer`；只有编辑入口打开全屏 overlay。
