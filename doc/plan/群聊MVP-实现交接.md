...
- **Orchestrator**：用群 vendor/model + 内置 prompt + `orchestrator_context{projectGoal,activeTaskGraph,recentUserIntents,blackboardSummary,memberStatus}` → 产出计划(单任务 or task_graph) → 写黑板 + 发 task-list 消息 → 串行 dispatch → 聚合汇报。计划生成走可注入 `OrchestratorPlanner` 接口。
