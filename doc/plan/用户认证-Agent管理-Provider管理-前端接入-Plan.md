...
- **`components/AgentCreateDialog.vue`** — 新建表单：name、vendor、platformProvider（下拉，取自 `providerApi.list`）、model（依所选 provider 的 modelList 联动）、workingDirectory，以及 systemPrompt / skills / mcp / allowedTools（按 vendor capabilities 动态启用/禁用——codex 不支持 systemPrompt/skills/mcp，前端按 capabilities 灰显并提示，避免提交后端报 BAD_REQUEST）。
