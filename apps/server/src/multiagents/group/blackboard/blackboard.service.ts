...
export class BlackboardService {
    ...
    async getState(groupId: string): Promise<BlackboardView> {
        const [artifacts, decisions, contracts, tasks] = await Promise.all([
            this.artifactRepo.find({ where: { groupChatId: groupId }, order: { updatedAt: 'DESC' } }),
            this.decisionRepo.find({ where: { groupChatId: groupId }, order: { createdAt: 'ASC' } }),
            this.contractRepo.find({ where: { groupChatId: groupId }, order: { contractKey: 'ASC' } }),
            this.taskRepo.find({ where: { groupChatId: groupId }, order: { seq: 'ASC' } })
        ])
        return toBlackboardView(artifacts, decisions, contracts, tasks)
    }
    ...
    async getArtifact(groupId: string, path: string): Promise<BlackboardArtifact | null> {
        ...
        return e ? toArtifactView(e) : null
    }
    ...
    async upsertArtifact(
        ...
        if (basedOnVersion !== undefined && basedOnVersion !== existing.version) {
            throw BusinessException.conflict(
                ...
                { reason: 'BLACKBOARD_CONFLICT', path: patch.path, currentVersion: existing.version }
            )
        }
    ...
    async getContract(groupId: string, contractKey: string): Promise<BlackboardContract | null> {
        ...
        return e ? toContractView(e) : null
    }
    ...
    async summarize(groupId: string): Promise<string> {
        ...
        if (state.artifacts.length) {
            ...
            for (const a of state.artifacts) {
                lines.push(`- [${a.status}] ${a.path} (v${a.version}, owner=${a.ownerAgentId}): ${a.summary}`)
            }
        }
