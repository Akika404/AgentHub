...
export class WorkspaceDiffService {
    ...
    async summarize(
        ...
        for (const entry of entries) {
            if (
                ...
                (!entry.oldPath || this.isIgnoredPath(entry.oldPath))
            ) {
                continue
            }
    ...
    async commit(
        ...
        try {
            ...
            throw BusinessException.badRequest(
                `Failed to commit workspace changes: ${this.errMsg(err)}`
            )
        }
    ...
    private async ensureGitRepository(workingDirectory: string): Promise<void> {
        try {
            ...
            throw BusinessException.badRequest(
                `Failed to prepare workspace git repository: ${this.errMsg(err)}`,
                { workingDirectory }
            )
        }
    }
    ...
    private async lineStats(
        ...
        try {
            const out = await this.git(workingDirectory, [
                'diff',
                '--numstat',
                baseRef,
                '--',
            ...
            if (entry.status === 'untracked' || !baseRef) {
                return await this.git(
                    ...
                    [
                        ...
                        '--',
                        '/dev/null',
                        resolve(workingDirectory, entry.path)
                    ],
                    [0, 1]
                )
            }
