...
> 本文是 server 的「目录结构 + 模块/接口文档」总览。后端开发规则（API 设计优先级、DTO/Entity 分离、统一异常等）仍在同目录的
> [`CLAUDE.md`](./CLAUDE.md) / [`AGENTS.md`](./AGENTS.md) 中，工具会自动加载；修改 server 代码前请先读它们。
...
│   ├── user_management.sql
│   ├── platform_provider.sql
│   ├── agent_manager.sql
│   └── group_chat_collaboration.sql
└── src/
    ...
    ├── redis/                        # @Global Redis 模块，导出 REDIS_CLIENT token
    ├── health/                       # 健康检查端点（探测 MySQL，Redis 探测当前注释）
    ├── chat-client/                  # 统一 LLM 客户端（OpenAI Responses/Completions + Anthropic）
    ├── user/                         # 用户管理 + JWT 认证基座（详见「用户管理模块」）
    ├── platform-provider/            # 用户自建模型平台（Provider）管理（详见「Provider 管理模块」）
    ├── user-workspace/               # 按用户隔离的服务器文件空间：skills/session/agent_home/agent_workspace
    ├── workspace-fs/                 # 服务端目录浏览 + 本地 Skill 上传（详见「服务端目录浏览模块」）
    └── multiagents/                  # Agent 配置 + 单聊 turn runtime + 群聊协作（详见「多 Agent 管理模块」「群聊协作模块」）
```
...
```
common/
├── index.ts                              # barrel（仅 re-export dto / exceptions / filter / interceptor）
├── dto/
│   ├── api-response.ts                    # ApiResponse 类型 + buildSuccess / buildFailure 信封构造器
│   └── api-response.dto.ts                # 统一信封的 Swagger 文档模型
├── exceptions/
│   ├── business.exception.ts             # 统一业务异常（携带 code/status 的工厂助手）
│   └── error-code.ts                     # 业务错误码枚举 + HTTP 状态映射
├── filters/all-exceptions.filter.ts      # 全局过滤器：把所有错误归一为响应信封
├── interceptors/response.interceptor.ts  # 把控制器成功返回包裹进统一信封
├── decorators/skip-envelope.decorator.ts # @SkipEnvelope 标记跳过信封的路由（如 SSE）
└── swagger/api-envelope.decorator.ts     # @ApiEnvelope 声明带信封的响应 schema
```

### `chat-client/` —— 统一 LLM 客户端

```
chat-client/
├── chat-client.ts            # 统一 LLM 调用：OpenAI Responses/Completions + Anthropic
├── chat-client.module.ts     # 导出 DefaultChatClient + CHAT_CLIENT 注入 token
└── index.ts                  # barrel
```

无状态的轻量 LLM 客户端，供 Orchestrator 隐藏交接判断、续编排判断等「不复用成员会话」的场景调用，不污染 `orchestratorSessionId`。
...
pnpm -F @agenthub/server test        # tsx + node:test 单元测试
```
...
- 其余 turn / 群运行相关开关见对应模块小节（`AGENT_RECLAIM_ON_BOOT`、`AGENT_TURN_TIMEOUT_MS`、`GROUP_RECLAIM_ON_BOOT`、`GROUP_MAX_ORCHESTRATION_STAGES`、`GROUP_DEBUG_LOGS`、`GROUP_DEBUG_LOG_MAX_CHARS`）。

> 远端部署时，Agent Home、单聊工作目录、群聊共享工作区、`skillSourceDirectories` 都是**后端服务器路径**。桌面端目录选择器通过 `/api/workspace-fs/*` 只能浏览当前登录用户自己的 `skills` / `agent_home` / `agent_workspace`；桌面端也可先读取用户本地 Skill 文件夹并上传到当前用户服务器 `skills` 根目录，再把返回的服务器路径作为 `skillSourceDirectories` 使用。

## 健康检查

启动后访问 `http://localhost:3000/api/health`，会 ping MySQL（`SELECT 1`）并返回聚合状态（Redis 探测当前已在源码中注释）。

## API 文档

- 交互式 API 文档（Scalar UI）在 `/api/reference`；原始 OpenAPI JSON 在 `/api/openapi.json`。可用 `API_DOCS_ENABLED=false` 关闭。
- 新增路由用 `@ApiTags` / `@ApiOperation` 标注，并用 `@ApiEnvelope(model)` 包裹响应，以反映统一的 `{ code, message, data, timestamp }` 信封；响应体需要 DTO **class**（interface 在运行时被擦除，会产出空 schema）。
...
│   └── agent-artifact.service.ts          # 从工作区 diff 快照推导单聊产出物
├── messages/
│   └── agent-message-history.service.ts   # UI 消息与运行步骤的读取/落库/清理 + 视图构建
├── workspace/
│   ├── agent-workspace.service.ts         # 工作目录/home 规范化、vendor 配置同步、skill 导入
│   └── workspace-diff.service.ts          # 会话工作区累计 diff / 提交 checkpoint（封装 WorkspaceGit）
├── context/
│   └── pinned-message-context.ts          # 把 pinned 消息渲染为预算受限的上下文块
├── local-runner/
│   ├── local-runner.gateway.ts            # WS 反向通道网关，转发 run/RPC 到桌面本地 runner
│   ├── remote-agent.adapter.ts            # 在用户桌面执行 turn 的 AgentAdapter
│   └── async-queue.ts                     # 单生产/单消费异步队列，push 事件 → for-await
├── adapter/                               # 统一适配层（重导出 @agenthub/agent-core）
│   ├── index.ts                           # forward 到 @agenthub/agent-core
│   ├── README.md                          # 统一 Claude/Codex 适配器 + 事件契约设计文档
│   └── test/                              # SSE demo server/页面 + CLI 示例（capabilities/demo.html、example.ts、server.ts）
├── entities/
│   ├── agent.entity.ts                    # Agent 配置
│   ├── agent-session.entity.ts            # 单 Agent / 群成员运行会话
│   ├── agent-message.entity.ts            # 按 sessionId 隔离的 UI 消息历史
│   └── agent-message-step.entity.ts       # agent 消息的有序运行步骤（thinking/progress/tool/todo）
├── dto/                                   # agents / agent-chats 入参、各 view 契约、OpenAPI 文档类
├── mappers/                               # agent / agent-message 实体 → 视图
└── group/                                 # 群聊协作模块（详见下文）
```
...
├── deployment/
│   ├── deployment-runner.interface.ts # DeploymentRunner 契约 + DI token + 日志/状态类型
│   ├── deployment.service.ts          # 部署生命周期 + 日志扇出订阅
│   └── local-process.runner.ts        # 以探测式本地子进程运行一次部署
├── run/
│   ├── group-run.executor.ts          # converse 入口 + 串行编排一次群运行 + abort
│   ├── group-run-stream.service.ts    # 群运行事件流（Redis Stream，沿用 turn-stream 范式）
│   ├── task-scheduler.ts              # 纯 DAG 调度：ready set + 下游阻塞
│   ├── orchestrator.service.ts        # 出计划 → 写 task_graph + 发 task-list → 聚合汇报
│   ├── orchestrator-executor.ts       # OrchestratorExecutor 接口 + LLM 默认实现（可注入假 Planner）
│   ├── orchestrator-final-reviewer.ts # 运行产出/产物对照原始需求的最终 LLM 审查
│   ├── orchestrator-handoff-reviewer.ts # 任务交接到下游前的隐藏 LLM 审查闸
│   ├── dispatch.service.ts            # 单次派发：装配→worktree→跑 turn→git diff 代写黑板→处理报告
│   └── member-chat.service.ts         # 轻量成员聊天 turn（无 task/worktree/report/diff）
...
- 10 张表（`group_chat` / `group_chat_member` / `group_message` / `group_run` / `blackboard_artifact` / `blackboard_decision` / `blackboard_contract` / `blackboard_task` / `blackboard_event` / `agent_memory_item`），dev 环境 TypeORM `synchronize` 自动建表，按 `userId` / `groupChatId` 隔离。结构存档见 `sql/group_chat_collaboration.sql`，**真实结构以实体定义为准**。
...
## 服务端目录浏览模块（`src/workspace-fs/`）与用户文件空间（`src/user-workspace/`）

桌面端连接远端后端时，不能使用 Electron 本机目录弹窗选择 Agent 工作区。`workspace-fs` 模块提供受 JWT 保护的服务器目录浏览 API，只返回当前登录用户文件空间内的一级子目录，用于选择 Agent Home、单聊工作目录、群聊共享工作区与 skill source folders；同时提供本地 Skill 文件夹上传入口，把桌面端读取到的本地文件清单写入当前用户服务器 `skills` 根目录。`user-workspace` 模块负责派生并保证每个用户的 `skills` / `session` / `agent_home` / `agent_workspace` 根目录存在。

### 目录结构

```
src/workspace-fs/
├── workspace-fs.module.ts
├── workspace-fs.controller.ts            # /workspace-fs：受 JWT 保护的目录浏览 + 本地 skill 上传
├── workspace-fs.service.ts               # realpath 归属校验 + 目录列举 + skill manifest 落盘
└── dto/workspace-fs-response.dto.ts

src/user-workspace/
├── user-workspace.module.ts
└── user-workspace.service.ts             # 解析/创建当前用户 skills/session/agent_home/agent_workspace 根
```
...
- 目录浏览 API 只浏览目录，不提供删除、重命名。新目录仍可在前端手动输入路径，但创建 Agent/聊天/群聊时会再次校验必须位于当前用户对应目录内。
- 本地 Skill 上传 API 不接收本机绝对路径，只接收桌面端读取后的相对路径文件清单；后端拒绝绝对路径、`..` 穿越、超限文件，并只写入当前用户的服务器 `skills` 根目录。

## 下一步建议

1. 在 `packages/shared` 里定义后端要暴露的接口契约，让 `apps/server` 直接 `import` 复用类型（部分已落地，见 `@agenthub/shared`）。
2. 把当前 MVP 的串行群聊编排扩展为 DAG 并行调度 + 冲突检测仲裁 + 失败重试降级（第二份 spec）。
