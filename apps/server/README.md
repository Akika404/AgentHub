...
桌面端连接远端后端时，不能使用 Electron 本机目录弹窗选择 Agent 工作区。`workspace-fs` 模块提供受 JWT 保护的服务器目录浏览 API，只返回当前登录用户文件空间内的一级子目录，用于选择 Agent Home、单聊工作目录、群聊共享工作区与 skill source folders；同时提供本地 Skill 文件夹上传入口，把桌面端读取到的本地文件清单写入当前用户服务器 `skills` 根目录。
