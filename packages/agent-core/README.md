...
**框架无关的 Agent 引擎**：把 Claude Agent SDK / OpenAI Codex SDK 归一为统一的 `AgentEvent` 流，并提供工作区 git diff/commit、产物预览与 text/html 产物写回能力。被 desktop 主进程（本地 runner）与 server（`multiagents/adapter` 重导出）共同复用，本身不依赖任何 Web 框架。
...
└── src/
    ├── index.ts                       # barrel：重导出 adapters、WorkspaceGit、artifact preview/writeback
    ...
    └── workspace/
        ...
        └── artifact-preview.ts         # 安全构建产物文件预览与 text/html 写回
```
